...
def plot(json_fname, results_fname, store_plots='', plots_to_latex=''):
    ...
    plt.plot(truely_detected_and_truely_corrected[0], truely_detected_and_truely_corrected[1], 'ro',
             ...
             falsely_detected[0], falsely_detected[1], 'go')

    fig = plt.gcf()
    # Change seaborn plot size
    fig.set_size_inches(12, 8)

    sns.displot(truely_detected_and_truely_corrected[0],  kind="kde",fill=True)
    plt.xlabel("Confidence")
    plt.title("The density of the confidence value with \"Truely detected and truely corrected data\"")
    plt.show()
    sns.displot(truely_detected_and_truely_corrected[1], kde="kde", fill=True)
    plt.xlabel("Similarity")
    plt.title("The density of the similarity value with \"Truely detected and truely corrected data\"")
    plt.show()

    sns.displot(truely_detected_and_falsely_corrected[0],  kind="kde",fill=True)
    plt.xlabel("Confidence")
    plt.title("The density of the confidence value with \"Truely detect and falsely corrected data\"")
    plt.show()
    sns.displot(truely_detected_and_falsely_corrected[1], kde="kde", fill=True)
    plt.xlabel("Similarity")
    plt.title("The density of the similarity value with \"Truely detect and falsely corrected data\"")
    plt.show()

    sns.displot(falsely_detected[0],  kind="kde",fill=True)
    plt.xlabel("Confidence")
    plt.title("The density of the confidence value  with \"Falsely detected\"")
    plt.show()
    sns.displot(falsely_detected[1], kde="kde", fill=True)
    plt.xlabel("Similarity")
    plt.title("The density of the similarity value with \"Falsely corrected\"")
    plt.show()
