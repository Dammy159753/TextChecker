...
errant_parallel_to_m2 = f"/media/Data/wangjunjie_code/fairseq_gec_Kakao/errant/gec_postprocess.py"
...
def parallel_to_m2(ori, cor, m2):
    p2m = f"python {errant_parallel_to_m2} -orig {ori} -cor {cor} -out {m2}"
    os.system(p2m)
...
def apply_lm_rerank(m2_entries, preserve_spell, max_edits, lm_scorer):
    ...
    for m2_entry in tqdm(m2_entries):
        ...
        for line in edits:
            ...
            if error_type == 'noop':
                preserve_m2.append(line)
            elif preserve_spell and "SPELL" in error_type:  # 是否改正拼写错误的
                preserve_m2.append(line)
            else:
                rerank_m2.append(line)  # 这个循环去掉了noop 和 SPELL 编辑
        ...
        if max_edits is None or 0 < len(rerank_m2) < max_edits:
            ...
            score_dict = lm_scorer.score(cor_sents)  # 不做改变的原始句子在位置0
            ...
            if min_idx != 0 and -(score_dict[min_idx] - score_dict[0])/score_dict[0] > 0.02:
                ...
                sorted_m2_lines = sort_m2_lines(preserve_m2 + list(edit_comb[0]))
            # sorted_m2_lines = sort_m2_lines(preserve_m2 + list(edit_comb[min_idx]))
            ...
            output_entries.append("\n".join([lines[0], noop]))  # error > max_edits 错误太多的，就直接返回
    return output_entries
